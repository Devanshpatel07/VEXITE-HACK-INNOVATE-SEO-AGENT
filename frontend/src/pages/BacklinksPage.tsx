...
import { Search, ExternalLink, Mail, Copy, Check, Sparkles, Shield, TrendingUp, SlidersHorizontal, Globe } from "lucide-react";
...
export default function BacklinksPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (projects && projects.length > 0) {
            ...
            const oppRes = await fetch(`/api/projects/${latestProject.project_id}/opportunities`);
            if (oppRes.ok) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const interval = setInterval(async () => {
          attempts++;
          const oppRes = await fetch(`/api/projects/${data.project_id}/opportunities`);
          if (oppRes.ok) {
  ...
  const filteredItems = opportunities.filter((item) => {
    ...
    if (item.score < minDa) return false;
    return true;
  });
    ...
    <div className="space-y-10 w-full max-w-6xl mx-auto pb-16">
      ...
      <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 max-w-3xl mx-auto">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3.5 bg-slate-900 border border-slate-800 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition text-sm"
          />
        </div>
        <button
          type="submit"
          disabled={loading || !keyword}
          className="px-8 py-3.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold rounded-xl flex items-center justify-center gap-2 transition shadow-lg shadow-purple-600/20"
        >
          {loading ? (
      ...
      <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <select
              ...
              className="bg-slate-950 border border-slate-700 text-slate-200 text-xs rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
        ...
        <div className="text-xs text-gray-400 font-medium">
          Showing <span className="text-purple-400 font-bold">{filteredItems.length}</span> of {opportunities.length} sites
        </div>
      </div>
      ...
      {filteredItems.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredItems.map((opp, idx) => (
            <div
              key={idx}
              className="p-6 bg-slate-900/80 border border-slate-800 hover:border-purple-700/50 rounded-2xl space-y-4 transition group hover:shadow-xl hover:shadow-purple-900/10 text-left"
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-xl font-bold text-white group-hover:text-purple-400 transition flex items-center gap-2">
                    {opp.domain}
                    <a
                      href={opp.url}
                      target="_blank"
                ...
                <div className="flex flex-col items-end">
                  <span className="text-2xl font-black text-purple-400">{opp.score}</span>
                  <span className="text-[10px] text-gray-500 uppercase tracking-widest">Domain Rating</span>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-2 pt-2 text-xs">
                <span className="px-2.5 py-1 rounded-md bg-purple-950/60 border border-purple-800/40 text-purple-300 font-medium flex items-center gap-1">
                ...
                <span className="px-2 py-0.5 rounded bg-indigo-950/60 border border-indigo-800/40 text-indigo-300 text-[10px] font-bold">
                  {opp.niche || "SaaS"}
                </span>
              </div>
                ...
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 relative">
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-gray-200 text-xs rounded flex items-center gap-1.5 transition"
                    >
        ...
        <div className="p-12 text-center bg-slate-900/60 border border-slate-800 rounded-3xl space-y-4 max-w-2xl mx-auto shadow-xl">
